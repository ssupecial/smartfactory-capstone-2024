...
class BasicJsspEnv(gym.Env):
    ...
    def __init__(
        ...
        if name is None:
            ...
            i, n, m, processing_time_matrix, machine_matrix = load_random(
                num_jobs, num_machines
            )
            tmp_random = np.random.randint(0, i - 1)
            processing_time_matrix = processing_time_matrix[tmp_random]
            machine_matrix = machine_matrix[tmp_random]
            machine_matrix = machine_matrix - 1

        else:
            self.name = name
            n, m, processing_time_matrix, machine_matrix = load_instance(name)

        # print(machine_matrix.shape, processing_time_matrix.shape)
        ...
        self.job_manager = JobSet(
            self.machine_matrix,
            self.processing_time_matrix,
            embedding_dim=self.embedding_dim,
        )
        self.machine_manager = MachineSet(
            self.machine_matrix, self.job_manager, self.delay, self.verbose
        )
        ...
        self.observation_space = BasicState(
            self.num_jobs, self.num_steps, self.num_machines
        )
    ...
    def reset(self, random_rate: float = 0.0, shuffle: bool = False, ret: bool = True):
        """
        ...
        if random_rate:
            if shuffle:
                for job_id in range(self.num_jobs):
                    ...
                    processing_time_matrix[job_id][idx_s] = processing_time_matrix[
                        job_id
                    ][idx_t]
            ...
            for job_id in range(self.num_jobs):
                for step_id in range(self.num_steps):
                    if np.random.random() < random_rate:
                        ...
                        processing_time_matrix[job_id][step_id] = int(
                            processing_time_matrix[job_id][step_id]
                        )

        # remake env
        self.job_manager = JobSet(
            machine_matrix, processing_time_matrix, embedding_dim=self.embedding_dim
        )
        self.machine_manager = MachineSet(
            machine_matrix, self.job_manager, self.delay, self.verbose
        )
        self.global_time = 0  # -1 matters a lot
        self.num_unsafety = 0
        self.num_do_action = 0
    ...
    def cal_reward(
        self, action: int = None, reward_type: str = None, doable_ops: list = None
    ):
        """
        ...
        if reward_type == "utilization":
            ...
            reward = -float(len(self.machine_manager.get_idle_machines())) / float(
                self.num_machines
            )

        else:
            raise KeyError(
                "Don't know what is {} reward. Please use 'utilization', 'idle_time', etc.".format(
                    reward_type
                )
            )
    ...
    def get_doable_ops_in_dict(
        self, machine_id: int = None, shuffle_machine: bool = True
    ):
        """
        ...
        if machine_id is None:
            ...
            if machine_id in available_machines:
                ...
                raise RuntimeWarning(
                    "Access to the not available machine {}. Return is None".format(
                        machine_id
                    )
                )
        return ret

    def get_doable_ops_in_list(
        self, machine_id: int = None, shuffle_machine: bool = True
    ):
        """
        Get all doable operations.
    ...
    def observe(self, return_doable: bool = True):
        ...
        Returns:
            ...
            * observation(DiGraph): one graph which describes the disjunctive graph and contained other info.
            * done(boolean): Whether it’s time to reset the environment again. True indicates the episode has terminated.
        """
        done = self.is_done()
    ...
    def eval_performance(self, performance: str = "unsatety ratio"):
        ...
        if performance == "unsafety ratio":
            ...
            for machine in self.machine_manager:
                ret["Machine {}".format(machine.machine_id)] = (
                    machine.total_work_time / machine.finish_time
                )
        else:
            raise KeyError(
                "Don't know what is {} performance. Please use 'makespan', 'utilization', etc.".format(
                    performance
                )
            )
    ...
    def draw_gantt_chart(self, path, benchmark_name, max_x=None):
        ...
        # Draw a gantt chart
        return self.job_manager.draw_gantt_chart(path, benchmark_name, max_x)

    def g2s(self, g: nx.DiGraph):
        """
        ...
        for n in nodelist:
            ...
            state[n * 10 + 6] = min(
                1.0, g.nodes[n]["waiting_time"] / self._max_pro_time
            )
    ...
    def render(self, mode="human"):
        ...
        return self.draw_gantt_chart("tmp.html", self.name)
...
class GraphJsspEnv(BasicJsspEnv):
    def __init__(
        ...
        self.observation_space = GraphState(
            self.num_jobs, self.num_machines, self.num_machines
        )

    def get_adjacent_matrix(self, g: nx.DiGraph) -> csr_matrix:
        """
    ...
    def get_disjunctive_matrix(self, g: nx.DiGraph) -> csr_matrix:
        """
        ...
        if not hasattr(self, "D"):
            ...
            self.D = csr_matrix((data, (row, col)), shape=(n * m, n * m))
    ...
    def reset(self, random_rate: float = 0.0, shuffle: bool = True, ret: bool = True):
        """
        Reset the environment.
        ...
        """
        super(GraphJsspEnv, self).reset(
            ret=False, random_rate=random_rate, shuffle=shuffle
        )
        ...
        observation = {"feature": feature, "A": A, "D": D}
        if ret:
            return observation

    def __get_feature_from_g(self, g: nx.DiGraph) -> np.ndarray:
        """
        ...
        for n in range(nx.number_of_nodes(g)):
            ...
            feature[6, n] = min(1.0, g.nodes[n]["waiting_time"] / self._max_pro_time)
...
class HeuristicJsspEnv(BasicJsspEnv):
    ...
    def get_machine_doable_action(self):
        ...
        # print(len(available_machines))
        # print(self.num_machines)
        for machine in available_machines:
            try:
                s[machine.machine_id - 1] = machine.doable_ops_id
            except:
                print(machine.doable_ops_id)
...
class HeuristicGraphJsspEnv(HeuristicJsspEnv):
    ...
    def reset(self, random_rate: float = 0.0, shuffle: bool = True, ret: bool = True):
        ...
        Returns:
            The initial observation or None.
        """
        super(HeuristicGraphJsspEnv, self).reset(
            ret=False, random_rate=random_rate, shuffle=shuffle
        )
...
class HeuristicAttentionJsspEnv(HeuristicJsspEnv):
    def __init__(
        ...
        self.observation_space = AttentionState(
            self.num_jobs, self.num_steps, self.num_machines
        )

    def reset(
        self, random_rate: float = 0.0, shuffle: bool = True, ret: bool = True
    ) -> np.ndarray:
        """
        Reset the environment.
        ...
        Returns:
            The initial observation or None.
        """
        super(HeuristicJsspEnv, self).reset(
            ret=False, random_rate=random_rate, shuffle=shuffle
        )
    ...
    def __get_feature_from_g(self, g: nx.DiGraph) -> np.ndarray:
        ...
        """
        feature = np.zeros(
            (self.num_jobs * self.num_steps, self.feature_dim), dtype=np.float64
        )
        ...
        for n in range(nx.number_of_nodes(g)):
            ...
            feature[n, 6] = min(1.0, g.nodes[n]["waiting_time"] / self._max_pro_time)
...
class BasicState(gym.Space):
    ...
    * 'machine'(int): Describe the machine on which the operation needs to be processed. 0 if the process cannot be done; The machine id starts at 1.
    ...
    def __repr__(self):
        return "Jssp State({}, {}, {})".format(
            self.num_jobs, self.num_steps, self.num_machines
        )

    def __eq__(self, other):
        return (
            isinstance(other, BasicState)
...
class GraphState(gym.spaces.Dict):
    ...
    * 'machine'(int): Describe the machine on which the operation needs to be processed. 0 if the process cannot be done; The machine id starts at 1.
    ...
    def sample(self):
        ...
        nodelist = [i for i in range(num_nodes)]
        feature = np.ndarray((self.num_feature, num_nodes), dtype=np.float64)
        data = []
        row = []
        col = []
        for n in nodelist:
        ...
        n = self.num_jobs
        m = self.num_steps
        A = csr_matrix((data, (row, col)), shape=(n * m, n * m))
        D = csr_matrix((data, (row, col)), shape=(n * m, n * m))
        sample = {
            "feature": feature,
            "A": A,
            "D": D,
    ...
    def __repr__(self):
        return "Jssp Graph State({}, {}, {})".format(
            self.num_jobs, self.num_steps, self.num_machines
        )

    def __eq__(self, other):
        return (
            isinstance(other, GraphState)
            and self.num_jobs == other.num_jobs
            and self.num_machines == other.num_machines
            and self.num_steps == other.num_steps
        )
...
class AttentionState(spaces.Box):
    ...
    * 'machine'(int): Describe the machine on which the operation needs to be processed. 0 if the process cannot be done; The machine id starts at 1.
    ...
    def __eq__(self, other):
        return (
            isinstance(other, AttentionState)
            and self.num_jobs == other.num_jobs
            and self.num_machines == other.num_machines
            and self.num_steps == other.num_steps
        )
